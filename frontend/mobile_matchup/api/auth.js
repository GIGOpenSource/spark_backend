...
import {
	getGoogleIdToken, getAppleIdentityToken, isFirebaseMock, isAppleSignInConfigured,
...
export async function apiGoogleLogin(data = {}) {
	...
	if (payload.id_token) {
		...
		return Promise.reject({ message: '需要 Google 原生 SDK 提供 id_token' })
	}
...
export async function apiAppleLogin(data = {}) {
	...
	if (payload.identity_token) {
		delete payload.email
		delete payload.nickname
	} else if (!isFirebaseMock() && isAppleSignInConfigured()) {
		return Promise.reject({ message: '需要 Apple 原生 SDK 提供 identity_token' })
	} else if (!isFirebaseMock() && !isAppleSignInConfigured()) {
		return Promise.reject({ message: 'Apple 登录未配置' })
	}
...
/** @deprecated prefer apiSmsSend — kept for older call sites */
export function apiPhoneSendCode(data = {}) {
	return apiSmsSend(data)
}

/** @deprecated prefer apiSmsVerify */
export function apiPhoneLogin(data = {}) {
	return apiSmsVerify(data)
}
...
export function apiInviteTrack(data = {}) {
	return postRequest('/auth/invite/track/', { app_id: APP_ID, ...data }, { showLoading: false, errorOutput: false })
}
...
/** WeChat OAuth / mini-program login stub */
export function apiWechatLogin(data = {}) {
	return postRequest('/auth/wechat/login/', { remember: true, app_id: APP_ID, ...data })
}

export function apiWechatBind(data = {}) {
	return postRequest('/auth/wechat/bind/', { app_id: APP_ID, ...data })
}

export function apiWechatUnbind() {
	return postRequest('/auth/wechat/unbind/', {})
}
