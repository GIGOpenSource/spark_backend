<template>
  <PageContainer :title="t('opsBanner.title')" :sub-title="t('opsBanner.subtitle')">
    ...
    <el-table :data="rows" style="width:100%" v-loading="loading">
      ...
      <el-table-column prop="title" :label="t('opsBanner.titleCol')" min-width="120" />
      <el-table-column prop="subtitle" :label="t('opsBanner.subtitleCol')" min-width="120" />
...
import { getOpsBanners, opsBannerAction } from '../api'
...
async function load() {
  ...
  try {
    ...
    const res = await getOpsBanners(params)
...
async function remove(row) {
  ...
  await opsBannerAction({ app_id: workspaceAppId(), action: 'delete', id: row.id })
