<template>
  <PageContainer :title="t('selectAdmin.title')" :sub-title="t('selectAdmin.subtitle')">
    ...
    <el-table :data="rows" style="width:100%" v-loading="loading">
      ...
      <el-table-column prop="note" :label="t('selectAdmin.note')" min-width="120" />
      ...
      <el-table-column v-if="hasActions" :label="t('common.actions')" width="200" fixed="right">
        <template #default="{ row }">
          <el-button link type="success" @click="approve(row)">{{ t('selectAdmin.select') }}</el-button>
          <el-button link type="danger" @click="reject(row)">{{ t('selectAdmin.reject') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getSelectAdmin, selectAction } from '../api'
...
async function load() {
  ...
  try {
    ...
    const res = await getSelectAdmin(params)
...
async function approve(row) {
  ...
  await selectAction({ app_id: workspaceAppId(), action: 'select', id: row.id })
...
async function reject(row) {
  await ElMessageBox.confirm(t('common.updated') + '?', { type: 'warning' })
  await selectAction({ app_id: workspaceAppId(), action: 'reject', id: row.id })
  ElMessage.success(t('common.updated'))
  load()
}
