...
export function isSmsMock() {
	...
	return !f.sms_configured
}

export function isAppleSignInConfigured() {
	const f = bootstrapFeatures()
	return !!f.apple_signin_configured
}
		...
		try {
			if (typeof plus !== 'undefined' && plus.payment) {
				plus.payment.getChannels((channels) => {
					...
					const finish = (list) => {
						const out = (list || []).map(mapReceipt).filter(Boolean)
						resolve(out)
					}
					try {
					...
					try {
						if (typeof ch.restoreCompletedTransactions === 'function') {
	...
	try {
		const token = await new Promise((resolve) => {
			uni.login({
				provider: 'apple',
				success: (res) => {
					const t = (res && (res.appleInfo && res.appleInfo.identityToken))
						|| (res && res.authResult && res.authResult.identityToken)
						|| (res && res.identityToken)
						|| ''
					resolve(t || '')
				},
				fail: () => resolve(''),
...
export function openManageSubscriptionsUrl() {
	const platform = detectStorePlatform()
	let url = ''
	if (platform === 'ios') {
	...
	uni.showToast({ title: 'Subscription link copied', icon: 'none' })
}
