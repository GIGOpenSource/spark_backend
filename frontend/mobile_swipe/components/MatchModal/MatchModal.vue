...
import { isWomenFirst } from '@/utils/productProfile.js'
...
const resolvedMode = computed(() => props.messagingMode || (isWomenFirst() ? 'women_first' : 'any'))

const canSendOpen = computed(() => {
	if (resolvedMode.value !== 'women_first') return true
	if (props.iAmOpener == null) return true
	return !!props.iAmOpener
})

const modeHint = computed(() => {
	if (resolvedMode.value === 'women_first' && !canSendOpen.value) {
		return 'She moves first — wait for her opener'
	}
	return ''
})
...
.hint { display:block; color:rgba(255,255,255,0.85); font-size:26rpx; margin: 8rpx 24rpx 16rpx; }
