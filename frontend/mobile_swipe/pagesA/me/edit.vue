<template>
	<view class="page">
		<view class="header">
			...
			<text class="title display-font">Edit profile</text>
		</view>
		...
		<view class="row-btns">
			<view class="mini-btn" @click="runSmartPhotos"><text>Smart Photos</text></view>
			<view class="mini-btn ghost" @click="goPreview"><text>{{ $t('me.preview') }}</text></view>
		</view>
		...
		<text class="sec">Opening Moves</text>
		<text class="hint">Icebreakers others can answer when you match</text>
		<view v-for="(om, oi) in openingMoves" :key="'om'+oi" class="prompt-slot">
			<input class="input" v-model="openingMoves[oi]" :placeholder="'Opening move ' + (oi + 1)" placeholder-class="ph" />
		</view>

		<text class="sec">Voice & video prompts</text>
		<text class="hint">Optional media prompts — waveforms show on your profile</text>
		<view class="media-row">
			<view class="media-card" @click="pickVoicePrompt">
				<text class="media-title">Voice prompt</text>
				<text class="media-sub">{{ voicePromptUrl ? 'Recorded · tap to replace' : 'Record a short answer' }}</text>
				<view v-if="voicePromptUrl" class="wave-preview">
					<view v-for="n in 12" :key="n" class="wbar" :style="{ height: (30 + (n * 7) % 50) + '%' }" />
				</view>
			</view>
			<view class="media-card" @click="pickVideoPrompt">
				<text class="media-title">Video prompt</text>
				<text class="media-sub">{{ videoPromptUrl ? 'Clip attached · tap to replace' : 'Add a short clip' }}</text>
			</view>
		</view>
		<input class="input" v-model="voicePromptQ" placeholder="Voice prompt question" placeholder-class="ph" />
		<input class="input" v-model="videoPromptQ" placeholder="Video prompt question" placeholder-class="ph" />
		...
		<text class="sec">Badges</text>
		<text class="hint">Separate from Interests — pick up to 5 personality badges</text>
		<view class="chips">
			<view
				v-for="b in badgeOptions"
				:key="b.id"
				class="chip"
				:class="{ on: badges.includes(b.id) }"
		...
		<text class="sec">Dating mode</text>
		<view class="chips">
			<view class="chip" :class="{ on: datingMode === 'date' }" @click="datingMode = 'date'"><text>Date</text></view>
			<view class="chip" :class="{ on: datingMode === 'bff' }" @click="datingMode = 'bff'"><text>BFF</text></view>
			<view class="chip" :class="{ on: datingMode === 'bizz' }" @click="datingMode = 'bizz'"><text>Bizz</text></view>
		</view>
		...
		<view class="row-btns">
			<view class="mini-btn" @click="connectOAuth('instagram')"><text>Connect Instagram</text></view>
			<view class="mini-btn" @click="connectOAuth('spotify')"><text>Connect Spotify</text></view>
		</view>
...
import { BADGE_CATALOG, setDatingMode } from '@/utils/productProfile.js'

const interestOptions = [
	'Travel', 'Art', 'Music', 'Coffee', 'Fitness', 'Movies',
	'Cooking', 'Photography', 'Reading', 'Gaming', 'Hiking', 'Dogs'
]
const badgeOptions = BADGE_CATALOG.filter((b) => !b.auto)
...
const openingMoves = ref(['', '', ''])
const voicePromptUrl = ref('')
const videoPromptUrl = ref('')
const voicePromptQ = ref('Ask me anything')
const videoPromptQ = ref('A day in my life')
...
onMounted(async () => {
	try {
		...
		badges.value = [...((u.lifestyle && u.lifestyle.badges) || [])]
		datingMode.value = (u.lifestyle && u.lifestyle.dating_mode) || 'date'
		...
		const om = (u.lifestyle && u.lifestyle.opening_moves) || []
		openingMoves.value = [0, 1, 2].map((i) => (om[i] && String(om[i])) || '')
		const vp = (u.lifestyle && u.lifestyle.voice_prompt) || {}
		voicePromptUrl.value = vp.url || ''
		voicePromptQ.value = vp.q || 'Ask me anything'
		const vid = (u.lifestyle && u.lifestyle.video_prompt) || {}
		videoPromptUrl.value = vid.url || ''
		videoPromptQ.value = vid.q || 'A day in my life'
...
function toggleInterest(t) {
	...
	} else if (interests.value.length < 10) {
		interests.value = [...interests.value, t]
	}
}

function toggleBadge(id) {
	if (badges.value.includes(id)) {
		badges.value = badges.value.filter((x) => x !== id)
	} else if (badges.value.length < 5) {
		badges.value = [...badges.value, id]
	} else {
		uni.showToast({ title: 'Max 5 badges', icon: 'none' })
	}
}
...
async function runSmartPhotos() {
	try {
		...
		const ids = (res.results && res.results.photo_ids) || []
		if (ids.length && photos.value.length) {
			const map = {}
			photos.value.forEach((p) => { map[p.id] = p })
			photos.value = ids.map((id) => map[id]).filter(Boolean).concat(
				photos.value.filter((p) => !ids.includes(p.id))
			)
		}
		uni.showToast({ title: 'Smart Photos applied', icon: 'none' })
	} catch (e) {
...
async function connectOAuth(provider) {
	try {
		...
		if (!url) {
			uni.showToast({ title: 'OAuth not configured', icon: 'none' })
			return
		}
		// #ifdef H5
		...
		uni.showToast({ title: 'Auth link copied', icon: 'none' })
	} catch (e) {
		uni.showToast({ title: (e && e.message) || 'Connect failed', icon: 'none' })
	}
}
...
async function save() {
	...
	const moves = openingMoves.value.map((s) => String(s || '').trim()).filter(Boolean)
	if (moves.length) life.opening_moves = moves
	if (badges.value.length) life.badges = badges.value.slice()
	life.dating_mode = datingMode.value || 'date'
	if (voicePromptUrl.value) {
		life.voice_prompt = { q: voicePromptQ.value || 'Ask me anything', url: voicePromptUrl.value }
	}
	if (videoPromptUrl.value) {
		life.video_prompt = { q: videoPromptQ.value || 'A day in my life', url: videoPromptUrl.value }
	}
	const links = {}
	...
	try {
		...
		setDatingMode(datingMode.value)
...
function pickVoicePrompt() {
	const r = uni.getRecorderManager && uni.getRecorderManager()
	if (!r) {
	...
	uni.showModal({
		title: 'Voice prompt',
		content: 'Record up to 30s. Tap OK to start, then stop from the system bar.',
		success: (m) => {
			if (!m.confirm) return
			r.onStop(async (res) => {
				...
				try {
					...
					voicePromptUrl.value = (body.results && body.results.url) || path
					uni.showToast({ title: 'Voice saved', icon: 'none' })
				} catch (e) {
					voicePromptUrl.value = path
					uni.showToast({ title: 'Saved locally', icon: 'none' })
				} finally {
					uni.hideLoading()
				}
			})
			r.start({ format: 'mp3', duration: 30000 })
		},
	})
}

function pickVideoPrompt() {
	uni.chooseVideo({
		...
		success: async (res) => {
			...
			try {
				...
				videoPromptUrl.value = (body.results && body.results.url) || path
				uni.showToast({ title: 'Video saved', icon: 'none' })
			} catch (e) {
				videoPromptUrl.value = path
				uni.showToast({ title: 'Saved locally', icon: 'none' })
			} finally {
				uni.hideLoading()
			}
		},
		fail: () => uni.showToast({ title: 'Video picker unavailable', icon: 'none' }),
	})
}
...
.sec {
	display:block; color:#B8860B; font-size:24rpx; font-weight:700;
...
.media-row { display:flex; flex-direction:row; margin-bottom:12rpx; }
.media-card {
	flex:1; background:#FFF8E1; border-radius:16rpx; padding:20rpx;
	border: 1px solid rgba(255,198,41,0.4); margin-right:12rpx;
}
.media-card:last-child { margin-right:0; }
.media-title { display:block; color:#111; font-size:26rpx; font-weight:700; margin-bottom:6rpx; }
.media-sub { display:block; color:#8A6D00; font-size:20rpx; }
.wave-preview {
	display:flex; flex-direction:row; align-items:flex-end; height:48rpx; margin-top:12rpx;
}
.wbar { flex:1; background:#FFC629; border-radius:999rpx; min-width:4rpx; }
.photos { display:flex; flex-direction:row; flex-wrap:wrap; margin-bottom:8rpx; }
.row-btns { display:flex; flex-direction:row; margin-bottom:20rpx; }
.row-btns > view + view, .row-btns > button + button { margin-left: 12rpx; }
.mini-btn {
	flex:1; background:#FFC629; border-radius:999rpx; padding:18rpx; text-align:center;
}
.mini-btn.ghost { background:#FFF8E1; border:1px solid rgba(255,198,41,0.45); }
.mini-btn text { color:#111; font-size:24rpx; font-weight:700; }
.photo {
	width: 210rpx; height: 280rpx; border-radius: 20rpx; overflow:hidden;
	margin-right: 16rpx; margin-bottom: 16rpx; position:relative; background:#FFF8E1;
	border: 1px solid rgba(255,198,41,0.35);
}
.img { width:100%; height:100%; }
.pending {
	position:absolute; left:8rpx; top:8rpx; background:rgba(255,198,41,0.95);
	border-radius:8rpx; padding:4rpx 10rpx;
}
.pending text { color:#111; font-size:18rpx; }
.del {
...
.add { display:flex; align-items:center; justify-content:center; border:1px dashed rgba(255,198,41,0.55); }
.add text { color:#B8860B; font-size:48rpx; }
.input, .area {
	background:#FFF8E1; border-radius:16rpx; padding:24rpx; color:#111; margin-bottom:16rpx; width:100%; box-sizing:border-box;
	border: 1px solid rgba(255,198,41,0.35);
}
.city-row {
...
.loc-btn {
	...
	background: rgba(255,198,41,0.25); border: 1px solid rgba(255,198,41,0.45);
}
.loc-btn text { color:#8A6D00; font-size:24rpx; font-weight:700; }
...
.chip {
	...
	background:#FFF8E1; border:1px solid rgba(255,198,41,0.35);
}
.chip text { color:#222; font-size:24rpx; }
.chip.on { background:#FFC629; border-color:#FFC629; }
.chip.on text { color:#111; font-weight:700; }
.btn { background: #FFC629; border-radius:999rpx; padding:26rpx; text-align:center; margin-top:20rpx; }
